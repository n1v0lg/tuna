/*
 * Code taken from https://github.com/louismullie/hom-js 
...
(function (isNode) {
  ...
  var PaillierService = function (jsbn) {
    ...
    var Utils = {
      ...
      random: function (bitLength) {
        ...
        /* TODO: don't need to use jsbn random number generation
           if this running on the node server */
        return new jsbn.BigInteger(bitLength, new jsbn.SecureRandom());
    ...
    var Paillier = {
      ...
      generateKeys: function(keySize) {
        ...
        var p1 = p.subtract(jsbn.BigInteger.ONE),
            q1 = q.subtract(jsbn.BigInteger.ONE);
      ...
      PublicKey: function(keySize, n) {
        ...
        // Calculate n + 1
        this.np1 = n.add(jsbn.BigInteger.ONE);
    ...
    Paillier.PublicKey.prototype = {
      ...
      encrypt: function(m) {
        ...
        // Ensure the input message is a jsbn.BigInteger.
        var m = this.convertToBn(m);
        
        return this.randomize(this.n.multiply(m)
               .add(jsbn.BigInteger.ONE).mod(this.n2));
      ...
      add: function(a,b) {
        ...
        return a.multiply(b).mod(this.n2);
        
      },
      
      /* Add encrypted ciphertext and plaintext */
      addPlain: function(a,b) {
        
        return a.multiply(this.np1.modPow(b, this.n2)).mod(this.n2);
        
      },

      /* Multiply encrypted ciphertext by plaintext */
      mult: function(a,b) {
      ...
      L: function (x) {
        ...
        return x.subtract(jsbn.BigInteger.ONE).divide(this.n);
      ...
      /* 
       * Convert a number or string to a jsbn.BigInteger
       */
      convertToBn: function (m) {
        
        if (typeof(m) == 'string'){
          m = new jsbn.BigInteger(m);
        } else if (m.constructor != jsbn.BigInteger) {
          m = new jsbn.BigInteger(parseInt(m).toString());
        }
  ...
  if (isNode) {
    ...
    module.exports = PaillierService;
  }
  else {
    // Browser
    window.paillier = PaillierService(jsbn);
  }
